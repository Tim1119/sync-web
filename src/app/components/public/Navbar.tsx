...
const navigationItems: NavigationItem[] = [
  ...
  // **MODIFICATION START**
  {
    ...
    dropdown: [
      ...
      { title: "Organization", url: "/services/organization-solution" },
    ],
  },
  // **MODIFICATION END**
...
const DropdownItem: FC<DropdownItemProps> = ({ item, pathname }) => {
  ...
  return (
    <div className="relative group/parent">
      {/* Parent Link/Button */}
      <div
        className={`text-sm font-medium transition-colors relative group/link font-[inter] flex items-center cursor-pointer ${
          isActive ? "text-[#113CFC]" : "text-white hover:text-[#113CFC]"
        }`}
      >
        {item.title}
        <ChevronDown
          className={`h-4 w-4 ml-1 transition-transform duration-200 ${isActive ? "text-[#113CFC]" : "text-white group-hover/link:text-[#113CFC]"} group-hover/parent:rotate-180`}
        />
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8 relative z-58">
        <div className="relative flex items-center h-20">
          ...
          <div className="ml-auto hidden lg:flex items-center gap-4">
            <Link href="https://account.buysync.co/user/home" className="py-2">
              <Button className="text-sm text-white hover:text-[#113CFC] transition-colors font-medium">
                Account
              </Button>
            </Link>
            <Link href="/cards" className="py-2">
              <Button
                style={{ width: "123.03px", height: "45px" }}
                className=" text-sm bg-[#113CFC]  hover:bg-[#0E33E0]  text-white shadow-lg transition-all duration-300 w-fit "
              >
                Buy Card
              </Button>
            </Link>
        ...
        <div className="p-6 space-y-2 h-full flex flex-col pt-24">
          ...
          {navigationItems.map((item) => {
            ...
            const isActive = isDropdown
              ? pathname === item.url ||
                item.dropdown.some((d) => pathname === d.url)
              : pathname === item.url;
              ...
              return (
                <div key={dropdownItem.title} className="w-full">
                  ...
                  <div
                    onClick={() =>
                      setOpenDropdown(
                        isDropdownOpen ? null : dropdownItem.title,
                      )
                    }
                  ...
                  >
                    <div className="flex items-center space-x-3">
                      {Icon && (
                        <Icon
                          className={`h-5 w-5 ${isActive ? "text-[#113CFC]" : "text-gray-400 group-hover:text-white"}`}
                        />
                      )}
                      <span
                        className={`text-base font-medium ${isActive ? "text-[#113CFC]" : "text-white"}`}
                      >
                        {dropdownItem.title}
                      </span>
                    </div>
                  ...
                  {/* Dropdown Content */}
                  <div
                    className={`overflow-hidden transition-all duration-300 ${isDropdownOpen ? "max-h-40 opacity-100" : "max-h-0 opacity-0"}`}
                  >
          ...
          <div className="mt-auto pt-8">
            <Link
              href="https://account.buysync.co/user/home"
              onClick={() => setMobileMenuOpen(false)}
            >
              <Button className="w-full bg-white/10 hover:bg-white/20 text-white mb-4 transition-all duration-300 h-12 rounded-lg">
                Account
              </Button>
            </Link>

            <Link href="/cards" onClick={() => setMobileMenuOpen(false)}>
              <Button className="w-full bg-[#113CFC] hover:bg-[#0E33E0] text-white shadow-lg transition-all duration-300 h-12 rounded-lg">
                Get Started
              </Button>
            </Link>
          </div>
