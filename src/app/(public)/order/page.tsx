  ...
  return (
    <div
      className={`w-full h-full rounded-xl p-4 shadow-xl transition-all duration-300
            ${
              isSelected
                ? "ring-4 ring-blue-500/80 scale-[1.02]"
      ...
      <div className="absolute inset-0 flex flex-col justify-between p-4">
        ...
        <div
          className={`text-white ${
            isVertical
              ? "absolute bottom-4 right-4 -rotate-90"
              : "flex justify-end"
          }`}
        >
              ...
              {currentStage >= step.id ? (
                ...
                <span
                  className={`text-sm font-semibold ${
                    currentStage === step.id ? "text-white" : "text-black"
                  }`}
                >
                  {step.id}
                </span>
      ...
      <div className="flex flex-col items-center md:flex-row justify-center gap-8 mb-16">
        {CARD_TEMPLATES.map((template) => (
          <div
            ...
            className={` bg-white p-6 rounded-2xl w-full max-w-[300px] shadow-2xl transition-all duration-300 border-2 cursor-pointer ${
              selectedId === template.id
                ...
                : "border-gray-700 hover:border-grey-800"
            }`}
          ...
          >
            ...
            <p className="text-black text-sm mt-1 mb-4">
...
// --- Stage 3: Payment ---
const Stage3Payment: React.FC<StageProps> = ({
  formData,
  nextStage,
  prevStage,
}) => {
...
export const CardForm = () => {
  ...
  // Redirect immediately - this route is not accessible
  useEffect(() => {
  ...
  // Return null while redirecting - route is not accessible
  // All rendering code is commented out below to disable this route
  return null;

  /* COMMENTED OUT - Route is disabled
  return (
